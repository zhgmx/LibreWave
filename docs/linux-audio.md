...
`Primed` means that offline startup checks are complete. It does not mean that a production endpoint or PCM path is ready. Tests can move the bridge to `Active` only when fake offline delivery is attached. A fault moves the attempt to `Faulted`. Teardown moves it to `Quiescing`, waits for the capture, graph, and playback owners to leave their boundaries, then moves it to `Stopped`.
...
The bridge is not called by daemon startup, the production ALSA workers, or the PipeWire graph seam. It does not install policy, publish a host node, or access live hardware. Production work must obtain the typed observations from ALSA and PipeWire, compose the existing owners, and connect the graph callback. The production resource state remains `GraphNotReady`.
...
- ALSA capture and playback progress populate the typed observations for the two independent estimators.
